...
def YOLO():
    ...
    configPath = "./cfg/trash.cfg"                                 # Path to cfg
    weightPath = "./backup/trash/training/trash_best.weights"                                 # Path to weights
    metaPath = "./data/obj.data"                                    # Path to meta data
    inputFileName = "./data/test/test.mp4"
    ...
    if metaMain is None:
        metaTemp = darknet.load_meta(metaPath.encode("ascii"))
        metaMain = [metaTemp.names[i].decode("ascii") for i in range(metaTemp.classes)]
    if altNames is None:
        try:
    ...
    cap = cv2.VideoCapture(inputFileName)                             # Local Stored video detection - Set input video
    frame_width = int(cap.get(3))                                   # Returns the width and height of capture video
    frame_height = int(cap.get(4))
    # Set out for video writer
    out = cv2.VideoWriter(                                          # Set the Output path for video writer
        outputFileName, cv2.VideoWriter_fourcc(*"MJPG"), 10.0,
